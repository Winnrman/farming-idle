...
function generateGrid() {
    ...
    for (let i = 0; i < 4; i++) { // Loop to create 4 rows
        let row = document.createElement('div');
        row.classList.add('row');
        for (let j = 0; j < 4; j++) { // Loop to create 4 cells in each row
...
function harvestCrop(coords) {
    ...
    for (let i = 0; i < tiles.length; i++) {
        ...
        if (tile.id === coords) {
            ...
            inventory[`${crop} seeds`] = (inventory[`${crop} seeds`] || 0) + 1; // Add 1 seed
            ...
            console.log(`Harvested ${yieldAmount} ${harvestedCrop} from tile ${coords}.`);
            return;
        }
    }
    updateInventory();
...
function updateMastery(crop) {
    const thresholds = [5, 15, 30]; // Harvest thresholds for levels 1, 2, 3
    const bonuses = masteryBonuses;

    let currentLevel = mastery[crop].level;
    let harvested = mastery[crop].harvested;

    if (currentLevel < thresholds.length && harvested >= thresholds[currentLevel]) {
        mastery[crop].level++;
        let newLevel = mastery[crop].level;
        console.log(`Mastery level up! ${crop} is now level ${newLevel}: ${bonuses[newLevel].description}`);
    }
}
...
function viewMastery() {
    ...
    for (let crop in mastery) {
        ...
        console.log(`${crop} - Level: ${progress.level}, Harvested: ${progress.harvested}`);
    }
}
...
function plantCrop(coords, crop) {
    ...
    for (let i = 0; i < tiles.length; i++) {
        ...
        if (tile.id === coords) {
            ...
            inventory[seedType]--;
            console.log(`Planted ${crop} on tile ${coords}. You have ${inventory[seedType]} ${seedType} left.`);
    ...
    for (let i = 0; i < tiles.length; i++) {
        ...
        if (growth < maxGrowth) {
            if (growthTimer > 0) {
                ...
                console.log(`Crop ${crop} on tile ${tile.id} is waiting. Timer: ${growthTimer - 1}`);
            } else {
                ...
                console.log(`Crop ${crop} on tile [${tile.id.split('')}] has grown to stage ${growth}/${maxGrowth}.`);
            }
        } else {
    ...
    if (cmd === 'hoe') {
        ...
        for (let i = 0; i < tiles.length; i++) {
            ...
            if (tile.id === coords) {
                ...
                let nearbyTiles = [`${x - 1}${y}`, `${x + 1}${y}`, `${x}${y - 1}`, `${x}${y + 1}`];
                for (let j = 0; j < nearbyTiles.length; j++) {
                    let nearbyTile = document.getElementById(nearbyTiles[j]);
                    if (nearbyTile && nearbyTile.classList.contains('water')) { // Check if tile exists and is water
                        nearbyWater = true;
                        break;
                    }
                }
    ...
    if(cmd == 'harvest'){
        ...
        for (let i = 0; i < tiles.length; i++) {
            ...
            if (tile.id === coords) {
                ...
                let growth = parseInt(tile.dataset.growth) || 0;
                if (growth < 3) {
                    console.log(`Crop ${crop} on tile ${coords} is not fully grown. Cannot harvest.`);
                    return;
                }
                tile.classList.remove('crop');
                tile.textContent = '';
                ...
                console.log(`You now have ${inventory[crop]} ${crop} and ${inventory[`${crop} seeds`]} ${crop} seeds.`);
                updateInventory();
                return { coords: coords, crop: crop };
            }
        }
...
function openShop() {
    ...
    for (let crop in cropConfig) {
        ...
        itemDiv.innerHTML = `
            <span>${crop} - Level: ${masteryLevel}, Harvested: ${harvested}</span>
...
function updateShopInventory() {
    ...
    for (let crop in inventory) {
        if (inventory[crop] > 0) {
            ...
            cropDiv.textContent = `${crop.charAt(0).toUpperCase() + crop.slice(1)}: ${inventory[crop]} available (Sell for $${cropPrices[crop]} each)`;
